...
public class Converter {
	...
	@Parameter(names = { "-m", "-metadata" }, description = "metadata mapping file", required = false)
	private String metadata = null;

	@Parameter(names = { "-f", "-format" }, description = "file format", required = true)
	private Formats format;

	private Configuration metadataConfiguration() throws Exception {
		Configuration configuration = null;
		if (metadata == null) {
			configuration = ADSC.configuration();
		} else {
			FileInputStream is = new FileInputStream(new File(metadata));
			configuration = Configuration.read(is);
		}
	...
	public void run() throws Exception {
		...
		switch (format) {
		case ADSC:
			f = new ADSC(metadataConfiguration(), infile);
			break;
		case CBF:
			// f = new CBF(infile);
			break;
